...
def processplot(n): 
  ...
  data = sdf.read(from_path1+'dist'+str(n).zfill(4)+".sdf",dict=True)
  header=data['Header']
  time=header['time']
  name = 'Electron_en'
  den = data['dist_fn/en/'+name[0:-3]].data*6.4e-6
  dist_x  = data['Grid/en/'+name[0:-3]].data[0]/(q0*1.0e6)
  plt.plot(dist_x,den/(dist_x[1]-dist_x[0]),'-',marker='^',color='salmon',linewidth=3,label='$\Delta x=1/50\mu m$')
...
#  plt.text(100,1e12,'>100MeV[nC]'+str(np.sum(den[dist_x>100])*1.6e-19/1e-9))
  ...
  data = sdf.read(from_path2+'dist'+str(n).zfill(4)+".sdf",dict=True)
  header=data['Header']
  time=header['time']
  name = 'Electron_en'
  den = data['dist_fn/en/'+name[0:-3]].data*6.4e-6
  dist_x  = data['Grid/en/'+name[0:-3]].data[0]/(q0*1.0e6)
  plt.plot(dist_x,den/(dist_x[1]-dist_x[0]),'-',marker='o',color='cyan',linewidth=3,label='$\Delta x=1/100\mu m$')
  ...
  fig.savefig(to_path+'nch_015_en_'+str(n).zfill(4)+'.png',format='png',dpi=100)
  plt.close("all")
