...
def processplot(n):
  ...
  to_path_0=['./jpg_l0600/' ,'./jpg_l0900/','./jpg_l1200/','./jpg_l1380/','./jpg_l1500/','./jpg_l1640/','./jpg_l1800/','./jpg_l2000/','./jpg_l2240/','./jpg_l2560/']
  
  ######### Parameter you should set ###########
  youwant = ['bz'] 
#  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
#  youwant =  ['Poynting_x','Poynting_y','Poynting_z','ey','bz','ex','ey_averaged','bz_averaged','electron_density','electron_ekbar','carbon_density','carbon_ekbar','electron_en','electron_theta_en','carbon_en','carbon_theta_en']#,'e_0_theta_en','e_1_theta_en','c_0_theta_en','c_1_theta_en','e_0_x_px','e_1_x_px','c_0_x_px','c_1_x_px']
  ...
  for name in youwant:
      for i in range(np.size(from_path_0)):
        ...
        if not os.path.exists(to_path):
            os.makedirs(to_path) 
        if (name[0:8] == 'Poynting'):
                data = sdf.read(from_path+"poynt"+str(n).zfill(4)+".sdf",dict=True)
                header=data['Header']
                time=header['time']
                ...
                X, Y = np.meshgrid(x, y)
                ex = data['Derived/Poynting Flux/'+name[-1]].data/(1/mu0*exunit*bxunit)
                ...
                cbar=plt.colorbar()
                cbar.set_label('Normalized Poynting flux [1/$\mu_0E_0B_0$]',fontdict=font)
