...
if __name__ == "__main__":
  ...
  C1_no  = (py0[0,0]**2+1.)**0.5 - (radt0-radpx0)
  y_max_no = (C1_no/alpha)**0.5/2/np.pi
  gg_no_0 = (px0**2+py0**2+1)**0.5
  
  gg_no = convert_g_m(gg_no_0)
  theta_no = np.arctan2(py0,px0)/np.pi*180
  R_no = gg_no_0-px0
  ...
  px0 = np.reshape(px0,(part_number,nsteps))
  py0 = np.reshape(py0,(part_number,nsteps))
  radt_rr  = np.reshape(radt0,(part_number,nsteps))
  radpx_rr  = np.reshape(radpx0,(part_number,nsteps))
  C1_rr  = (py0[0,0]**2+1.)**0.5 - (radt_rr-radpx_rr)
  y_max_rr = (C1_rr/alpha)**0.5/2/np.pi
  gg_rr_0 = (px0**2+py0**2+1)**0.5


  gg_rr = convert_g_m(gg_rr_0)
  theta_rr = np.arctan2(py0,px0)/np.pi*180
  R_rr = gg_rr_0-px0
  ...
  px0 = np.reshape(px0,(part_number,nsteps))
  py0 = np.reshape(py0,(part_number,nsteps))
  radt_qe  = np.reshape(radt0,(part_number,nsteps))
  radpx_qe  = np.reshape(radpx0,(part_number,nsteps))
  C1_qe  = (py0[0,0]**2+1.)**0.5 - (radt_qe-radpx_qe)
  y_max_qe = (C1_qe/alpha)**0.5/2/np.pi
  gg_qe_0 = (px0**2+py0**2+1)**0.5
  ww  = np.zeros_like(gg_qe_0)+1

  gg_qe = convert_g_m(gg_qe_0)
  theta_qe = np.arctan2(py0,px0)/np.pi*180
  R_qe = gg_qe_0-px0
...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(t0[n,:]*3.333, C1_no[n,:], '-',color='k',linewidth=3,label='Without RR')
  plt.plot(t0[n,:]*3.333, C1_rr[n,:], '-',color='b',linewidth=3,label='Classic RR')
  for n in range(5000):
      if n ==0:
          plt.plot(t1[n,:]*3.333, C1_qe[n,:],'-',color='r',linewidth=0.2,zorder=0,alpha=0.5,label='QED RR')
      else:
          plt.plot(t1[n,:]*3.333, C1_qe[n,:],'-',color='r',linewidth=0.02,zorder=0,alpha=0.2)
 ...
 #   plt.legend(loc='upper right')
  ...
  plt.ylabel(r'$C_1$',fontdict=font)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  plt.subplots_adjust(left=0.15, bottom=0.16, right=0.95, top=0.98,
  ...
  fig.savefig('C1_5000_t.png',format='png',dpi=160)
  plt.close("all")
