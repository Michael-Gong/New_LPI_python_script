...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(t0[n,:]*3.333, R_no[n,:], '-',color='k',linewidth=3,label='Without RR')
  plt.plot(t0[n,:]*3.333, R_rr[n,:], '-',color='b',linewidth=3,label='Classic RR')
  for n in range(5000):
      if n ==0:
          plt.plot(t1[n,:]*3.333, R_qe[n,:],'-',color='r',linewidth=0.2,zorder=0,alpha=0.5,label='QED RR')
      else:
          plt.plot(t1[n,:]*3.333, R_qe[n,:],'-',color='r',linewidth=0.02,zorder=0,alpha=0.2)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  fig.savefig('R_5000.png',format='png',dpi=160)
  plt.close("all")
