...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(xi_no[n,:], R_no[n,:], '-',color='k',linewidth=3,label='Without RR')
  plt.plot(xi_rr[n,:], R_rr[n,:], '-',color='b',linewidth=3,label='Classic RR')
  for n in range(5000):
      plt.plot(xi_qe[n,:], R_qe[n,:],'-',color='r',linewidth=0.01,zorder=0)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  fig.savefig('R_5000_xi.png',format='png',dpi=160)
  plt.close("all")
