...
if __name__ == "__main__":
  ...
  radt_no = np.reshape(radt0,(part_number,nsteps))
...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(t0[n,:], radt_no[n,:]*0.51,'-',color='k',linewidth=3,label='Without RR')
  plt.plot(t0[n,:], radt_rr[n,:]*0.51,'-',color='b',linewidth=3,label='Classic RR')
  for n in range(5000):
      plt.plot(t1[n,:], radt_qe[n,:]*0.51,'-',color='r',linewidth=0.01, zorder=0)
 ...
 #   plt.legend(loc='upper right')
  ...
  plt.ylabel(r'$\varepsilon_\gamma\ [MeV]$',fontdict=font)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  fig.savefig('radt_5000.png',format='png',dpi=160)
  plt.close("all")
