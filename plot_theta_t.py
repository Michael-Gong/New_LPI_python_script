...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(t0[n,:], theta_no[n,:], '-',color='k',linewidth=1,label='Without RR')
  plt.plot(t0[n,:], theta_rr[n,:], '-',color='b',linewidth=2,label='Classic RR')
  for n in range(5000):
      plt.plot(t1[n,:], theta_qe[n,:],'-',color='r',linewidth=0.01,zorder=0)
...
#  plt.ylim(0,15)
  ...
  plt.ylabel(r'$\theta_e\ [^\circ]$',fontdict=font)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  fig.savefig('theta_5000.png',format='png',dpi=160)
  plt.close("all")
