...
if __name__ == "__main__":
  ...
  x_no = np.reshape(x0,(part_number,nsteps))
  y_no = np.reshape(y0,(part_number,nsteps))
  ...
  y0 = np.loadtxt(from_path+'y_0000.txt')/2/np.pi
  t0  = np.reshape(t0,(part_number,nsteps))
  x_rr = np.reshape(x0,(part_number,nsteps))
  y_rr = np.reshape(y0,(part_number,nsteps))
  ...
  x_qe = np.reshape(x0,(part_number,nsteps))
  y_qe = np.reshape(y0,(part_number,nsteps))
...
#  cbar.set_label('h [%]',fontdict=font)
  ...
  plt.plot(x_no[n,:], y_no[n,:],'-',color='k',linewidth=3,label='Without RR')
  plt.plot(x_rr[n,:], y_rr[n,:],'-',color='b',linewidth=3,label='Classic RR')
  for n in range(5000):
      plt.plot(x_qe[n,:], y_qe[n,:],'-',color='r',linewidth=0.01, zorder=0)
...
#  plt.title('t='+str(round(t0[0,i],0))+' $T_0$',fontdict=font)
  ...
  fig.savefig('x_y_5000.png',format='png',dpi=160)
  plt.close("all")
